...
export const metadata: Metadata = {
  ...
  description: 'Learn about our mission at UtilityHub: to build high-performance, private, client-side tools and calculators for everyday digital needs.',
};
...
export default function AboutPage() {
  ...
  const values = [
    ...
    },
    {
      ...
      desc: 'By compiling and pre-rendering our utilities statically with Next.js, we eliminate load times. Calculators render and respond instantly to custom parameters, sliders, and form adjustments.',
    },
    {
      icon: '💎',
    ...
    <div className="max-w-4xl mx-auto px-4 py-16 sm:px-6 lg:px-8 space-y-16">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <div
            key={i}
            className="p-6 bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-850 rounded-2xl text-center shadow-sm"
          >
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {values.map((val, i) => (
            <div
              key={i}
              className="p-6 bg-white dark:bg-zinc-900 border border-zinc-250/60 dark:border-zinc-800 rounded-3xl space-y-4 shadow-sm"
            >
      ...
      <section className="p-8 sm:p-12 bg-gradient-to-tr from-violet-600/10 to-indigo-500/10 dark:from-violet-950/20 dark:to-indigo-900/10 border border-violet-500/20 dark:border-violet-900/20 rounded-3xl text-center space-y-6">
        ...
        <div>
          <Link
            href="/"
            className="inline-block px-6 py-3 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-xs font-bold shadow transition-all"
          >
            Explore Dashboard
          </Link>
        </div>
