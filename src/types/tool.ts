...
export interface ToolField {
  ...
  options?: { label: string; value: string | number }[]; // For selects
  unit?: string; // e.g. "years", "%", "INR"
  slider?: { min: number; max: number; step: number }; // For visual range sliders
  helpText?: string;
  placeholder?: string;
}
...
export interface Tool {
  ...
  // Custom React Component replacement for complex tools (e.g. PDF split, JSON parser)
