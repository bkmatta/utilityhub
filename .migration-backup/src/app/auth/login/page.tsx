...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulated authentications login
    setTimeout(() => {
      setIsLoading(false);
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-8 shadow-sm space-y-6">
        <div className="text-center space-y-2">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-violet-600 to-indigo-500 flex items-center justify-center text-white font-black text-lg mx-auto shadow-sm">
            U
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl text-sm focus:ring-2 focus:ring-violet-500 focus:outline-none"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-bold shadow-md transition-all text-sm disabled:opacity-50"
          >
        ...
        <button
          onClick={() => {
            ...
            setTimeout(() => {
              setIsLoading(false);
              localStorage.setItem('user_session', JSON.stringify({ email: 'google.user@gmail.com', name: 'Google User' }));
              window.location.href = '/';
            }, 800);
          }}
          className="w-full py-2.5 border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-950 font-semibold text-zinc-700 dark:text-zinc-300 rounded-xl transition-all text-sm flex items-center justify-center gap-2"
        >
