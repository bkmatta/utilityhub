          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs sm:text-sm">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <input type="text" required placeholder="Your name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full px-4 py-2.5 border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50 dark:bg-zinc-950 focus:ring-2 focus:ring-violet-500 focus:outline-none" />
              </div>
              <div className="space-y-2">
                <label className="font-semibold text-zinc-700 dark:text-zinc-300">Email *</label>
                <input type="email" required placeholder="yourname@example.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full px-4 py-2.5 border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50 dark:bg-zinc-950 focus:ring-2 focus:ring-violet-500 focus:outline-none" />
              </div>
            </div>
            <div className="space-y-2">
              <label className="font-semibold text-zinc-700 dark:text-zinc-300">Subject</label>
              <select value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} className="w-full px-4 py-2.5 border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50 dark:bg-zinc-950 focus:ring-2 focus:ring-violet-500 focus:outline-none cursor-pointer">
            ...
            <div className="space-y-2">
              ...
              <textarea required rows={4} placeholder="Type your message here..." value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className="w-full px-4 py-2.5 border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50 dark:bg-zinc-950 focus:ring-2 focus:ring-violet-500 focus:outline-none resize-none" />
            </div>
            <button type="submit" disabled={isSubmitting} className="w-full px-6 py-3 bg-violet-600 hover:bg-violet-700 disabled:opacity-55 text-white font-bold rounded-xl shadow transition-all cursor-pointer">
        ...
        <div className="md:col-span-5 space-y-6">
          <div className="p-6 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl space-y-4">
            ...
            <div className="space-y-3 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
              <p><strong>Email Support:</strong><br /><a href="mailto:support@utilityhub.com" className="text-violet-600 dark:text-violet-400 hover:underline">support@utilityhub.com</a></p>
              <p><strong>Response Time:</strong><br />We aim to reply to all queries within 24 hours.</p>
              <p><strong>Bug Reports:</strong><br />Be sure to include the tool slug and details about your input parameters if reporting a calculation error.</p>
            </div>
          </div>
          <div className="p-6 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl space-y-4">
            ...
            <p className="text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
              ...
              <a href="mailto:security@utilityhub.com" className="text-violet-600 dark:text-violet-400 hover:underline font-semibold">security@utilityhub.com</a>.
            </p>
          </div>
        </div>
      </section>
