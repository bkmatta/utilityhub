...
export default function PdfWorkspace({ slug }: PdfWorkspaceProps) {
  ...
  const executeProtect = async () => {
    ...
    try {
      ...
      // Re-save with encryption using pdf-lib's built-in user/owner password support
      const encryptedDoc = await PDFDocument.load(savedPdfBytes);
      const protectedPdfBytes = await encryptedDoc.save({
      ...
      setSuccessMsg('PDF protected with password successfully.');
    } catch (err: any) {
