...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/80 dark:bg-zinc-950/80 backdrop-blur border-b border-zinc-200 dark:border-zinc-800 transition-colors">
                  ...
                  <div className="bg-white/95 dark:bg-zinc-900/95 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-xl p-3 grid grid-cols-1 gap-1 backdrop-blur-md">
                    {tools.slice(0, 8).map((tool) => (
                      ...
                      >
                        ...
                        <span className="text-[10px] text-zinc-400 dark:text-zinc-500 font-light truncate mt-0.5">{tool.description}</span>
                      </Link>
                    ))}
                    {tools.length > 8 && (
                      <Link
